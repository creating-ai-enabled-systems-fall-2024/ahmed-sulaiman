...
class DocumentProcessing:
    ...
    def fixed_length_chunking(self, text, fixed_length, overlap_size=2):
        words = text.split()
        chunks = []

        for start in range(0, len(words), fixed_length - overlap_size):
            chunk = words[start:start + fixed_length]
            if len(chunk) > 0:
